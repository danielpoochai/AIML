    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                allPossible[newPos] = prob*self.beliefs[oldPos] + allPossible[newPos]
...
class ParticleFilter(InferenceModule):
        ...
        while count < self.numParticles:
            for position in self.legalPositions:
                if count < self.numParticles:
                    self.list_particles.append(position)
                    count = count + 1
                else:
                    break
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        if noisyDistance == None:
            self.list_particles = [self.getJailPosition()] * self.numParticles
        else:
            allPossible = util.Counter()
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                allPossible[p] = allPossible[p] + emissionModel[trueDistance]*self.getBeliefDistribution()[p]
            if allPossible.totalCount() == 0:
                self.initializeUniformly(gameState)
                for p in range(len(self.list_particles)):
                    if noisyDistance == None:
                        self.list_particles[p] = self.getJailPosition()
            else:
                allPossible.normalize()
                items = sorted(allPossible.items())
                distribution = [i[1]for i in items]
                values = [i[0] for i in items]
                particle_tmp = util.nSample(distribution,values,self.numParticles)
                self.list_particles = particle_tmp
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.list_particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState,oldPos))
            for newPos, prob in newPosDist.items():
                allPossible[newPos] = prob*self.getBeliefDistribution()[oldPos] + allPossible[newPos]
        items = sorted(allPossible.items())
        distribution = [i[1] for i in items]
        values = [i[0] for i in items]
        self.list_particles = util.nSample(distribution,values,self.numParticles)
        #util.raiseNotDefined()
    ...
    def getBeliefDistribution(self):
        ...
        for particles in self.list_particles:
            beliefs[particles] += 1
        beliefs.normalize()
        return beliefs
        #util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        ghost_len = self.numGhosts
        prison = self.getJailPosition
    
        if self.numParticles > position_len**ghost_len:
            check = True
        else:
            check = False
        if not check: #particles <= #permutation
            while count < self.numParticles:
                particle_tmp = []
                numsuc = 0
                while numsuc < self.numGhosts:
                    particle_tmp.append(random.choice(position))
                    numsuc += 1
                particle = tuple(particle_tmp)
                if not counter.has_key(particle): #hasn't been added
                    counter[particle] = 1
                    self.particles.append(particle)
                    count += 1
            random.shuffle(self.particles)
        else: #particles > permutation:
            joint_particles = list(itertools.product(self.legalPositions,repeat = self.numGhosts))
            random.shuffle(joint_particles)
            while count < self.numParticles:
                for positions in joint_particles:
                    if count < self.numParticles:
            ...
            while count < self.numParticles:
                ...
                while numsuc < self.numGhosts:
                    ...
                    numsuc += 1
                particle = tuple(particle_tmp)
                if  not counter.has_key(particle): #hasn't been add
                    counter[particle] = 1
                    self.particles.append(particle)
                    count += 1
                else: #has been add
                    if len(self.particles)%(position_len**ghost_len) == 0: 
                        counter[particle] += 1
                        self.particles.append(particle)
                        count += 1
                    elif not counter[particle] == max(counter.values()):
                        counter[particle] += 1
                        self.particles.append(particle)
                        count += 1
            random.shuffle(self.particles)
        ...
        while count < self.numParticles:
            if count <self.numParticles:
                ...
                for i in range(self.numGhosts):
                    p.append(random.choice(position))
                p = tuple(p)
                count = count + 1
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                   p = self.getParticleWithGhostInJail(p,i)
                    ...
                    p_prob = p_prob*emissionModels[i][trueDistance]
            allPossible[p] += p_prob
            #*self.getBeliefDistribution()[p]
            #allPossible.normalize()
        if allPossible.totalCount() == 0:
            self.initializeParticles()
            ...
            distribution = [i[1]for i in items]
            values = [i[0] for i in items]
            particles_tmp = util.nSample(distribution,values,self.numParticles)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if newParticle[i] == self.getJailPosition:
                    newParticle = self.getParticleWithGhostInJail(newParticle[i],i) 
                    break
