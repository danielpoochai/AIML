...
class ValueIterationAgent(ValueEstimationAgent):
                ...
                if not self.mdp.isTerminal(state):
                    ...
                    for action in self.mdp.getPossibleActions(state):
                        ...
                        for nextstate , prob in self.mdp.getTransitionStatesAndProbs(state,action):
                            q_values += prob* (self.discount*copy_values[nextstate] + self.mdp.getReward(state,action,nextstate))
                        Q.append(q_values)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        q_value = 0
        for nextstate,prob in self.mdp.getTransitionStatesAndProbs(state,action):
            q_value += prob*(self.discount*self.values[nextstate]+self.mdp.getReward(state,action,nextstate))
        return q_value
        #else:
        #  return 0
    ...
    def computeActionFromValues(self, state):
        ...
        if self.mdp.isTerminal(state):
            ...
            for action in actions:
                value_tmp = []
                for nextstate,prob in self.mdp.getTransitionStatesAndProbs(state,action):
                    value_tmp.append(prob*self.values[nextstate])
                action_value[action] =  sum(value_tmp) 
