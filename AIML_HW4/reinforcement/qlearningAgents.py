...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if not legalactions :
            ...
            for action in legalactions:
                record_value.append(self.getQValue(state,action))
            return max(record_value)
    ...
    def computeActionFromQValues(self, state):
        ...
        for action in self.getLegalActions(state):
            if self.getQValue(state,action)> best_value or best_action == None:
    ...
    def update(self, state, action, nextState, reward):
        ...
        if not legalactions:
            ...
            second_part = self.alpha*(reward + self.discount * self.computeValueFromQValues(nextState))
        self.q_value[(state,action)] = first_part + second_part
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        if not actions:
            ...
            difference = reward + self.discount * max([self.getQValue(nextState,Action) for Action in actions]) - self.getQValue(state,action)
        for feature in features:
